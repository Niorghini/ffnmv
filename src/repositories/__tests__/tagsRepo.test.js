/**
 * tagsRepo 测试
 * - findOrCreate: 去重 + 复用 + 颜色稳定
 * - merge: 关联改向 + source 软删
 * - countsByTag: 只数未删 note + 未删 link
 */
...
describe('tagsRepo', () => {
  ...
  it('merge 关联改向 + source 软删', async () => {
    ...
    // 所有 n1/n2 关联改到 dst；n3 原本就有 dst，src 那条删掉
    ...
    expect(linksN1Src.deleted_at).toBeTruthy() // 软删
    ...
    expect(linksN3Src.deleted_at).toBeTruthy() // 软删
  })
  ...
  it('countsByTag 只数未删', async () => {
    ...
    const n1 = await notesRepo.create({ content: 'a', tagIds: [t1.id] })
    const n2 = await notesRepo.create({ content: 'b', tagIds: [t1.id, t2.id] })
